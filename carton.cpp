...
Carton::Carton(Address address, double weight, double side1, double side2, double side3) : ShippingItem(address, weight, side1)
{
  try
  {
      set_measurements(side1,side2, side3);
...
{
 out << std::fixed << "Carton:" << " " << std::setprecision(1) << weight_ << " lbs." << " " << length_ << " x " << width_ << " x " << height_ << endl;
}
//weight1_ = 7;
/**
...
void Carton::WriteData(std::ostream& out) const {
  out << weight_ << " " << length_ << " " << width_ << " " << height_ << endl;
}
/**
